...
roipth= '.../smallvolume_face_roi.mat';

roistruct = load(roipth);
roiidx= roistruct.voxelcordidx2;
roisize =size(roiidx,2);
...
results = zeros(arraysize,roisize);

faceintensities = zeros(arraysize,roisize);
nfaceintensities = zeros(1,arraysize);

scrambledintensities = zeros(arraysize,roisize);
...
while i <= upperboundary
    shift = i;
    index = round( i/timescale+1);%this is kinda wrong
    fprintf('%02d \n',i);
    ...
    % current shift
    [myvoxeldifference,faceScansVoxelIntensity,scrambledScansVoxelIntensity,coordinates,famousscans,unfamiliarscans,scrambledscans,famousscanplaces,unfamiliarscanplaces, scrambledscanplaces]= voxeldiffROI(outpth,scanpth,roiidx,roisize);
   ...
   nfaceintensities(index) = faceintsize(1);
    ...
    nscrambledintensities(index) = scrambledintsize(1);
...
if  i >= upperboundary
    ...
    if ~exist(resultsfoldername)
        ...
        filename = sprintf('brute_force_p%02d_roi_%.4fs_t%02d-%02d.mat',subject , timescale,lowerboundary, upperboundary);
